...
from .relativistic_wavelength import calculate_relativistic_electron_wavelength
...
def calculate_ctf_2d(
    ...
    grid = fftfreq_grid(image_shape=image_shape, rfft=rfft)  # (h, w, 2)
    grid = grid / einops.rearrange(pixel_size, 'b -> b 1 1 1')
    grid_squared = grid ** 2
    ...
    yy2, xx2 = einops.rearrange(grid_squared, 'b h w freq -> freq b h w')
    xy = einops.reduce(grid, 'b h w freq -> b h w', reduction='prod')
    n4 = einops.reduce(grid_squared, 'b h w freq -> b h w', reduction='sum') ** 2
